...
<div class="container mt-4">
    ...
    <form action="{{ route('lugarTuristico.update', $lugarTuristico->id) }}" method="POST" id="frm_lugar_turistico">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="descripcion" name="descripcion" rows="3">{{ $lugarTuristico->descripcion }}</textarea>
        </div>

        <div class="mb-3">
            <label for="anio" class="form-label">Año</label>
            <input type="text" class="form-control" id="anio" name="anio" value="{{ $lugarTuristico->anio }}">
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="accesibilidad" name="accesibilidad">
                <option value="Fácil" {{ $lugarTuristico->accesibilidad == 'Fácil' ? 'selected' : '' }}>Fácil</option>
                <option value="Medio" {{ $lugarTuristico->accesibilidad == 'Medio' ? 'selected' : '' }}>Medio</option>
                <option value="Difícil" {{ $lugarTuristico->accesibilidad == 'Difícil' ? 'selected' : '' }}>Difícil</option>
            </select>
        </div>
        ...
        <input type="hidden" id="coordenadas" name="coordenadas" value="{{ $lugarTuristico->coordenadas }}">
...
<script>
    function initMap() {
        ...
        var coordenadasGuardadas = document.getElementById("coordenadas").value;
        
        // Separar latitud y longitud
        var coordenadasArray = coordenadasGuardadas.split(',');
        var latitud = parseFloat(coordenadasArray[0]) || -0.9374805;
        var longitud = parseFloat(coordenadasArray[1]) || -78.6161327;
        ...
        // Establecer valores iniciales en los campos de latitud y longitud
